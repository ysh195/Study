 ...
 * 나는 그냥 연결관계를 일자로 쫙 펼쳤을 때 일렬로 연결된 친구 5명이 있냐는 것으로 이해함
 *
 * 입력 :
 ...
 * 
 * 출력 :
 * 5명 이상이 포함된 친구관계가 존재하면 1, 존재하지 않으면 0을 출력한다.
 * 
 * 풀이 :
 * 1. 숫자 입력에 사용할 BufferedReader를 준비한다.
 * 2. 띄어쓰기로 구분된 숫자 2개를 문자열로 입력 받고, 바로 " "를 기준으로 구분해 배열로 저장한다.
 * 3. 배열의 값을 숫자로 변환해서 첫번째 값은 N에 저장하고, 두번째 값은 M에 저장한다. 두 값을 제한 범위에 맞추어 조정한다.
 ...
 * 8-6-2. movecount가 5 이상인지 시작 시점에서 체크한다.
 *        만약 5이상이면, tf = 1로 하고 즉시 반복문을 통한 탐색을 모두 중지한다.
