 ...
 *    의외로 처음 시작 숫자들을 뭘로 설정할 지가 문제가 된다.
 *    매번 시작할 때마다 비어있지 않으면서 아직 탐색하지 않은 값을 for문으로 찾으면 실행횟수 소모가 극심하다.
 *    그렇다고 숫자 입력할 때 같이 받으면 중복이 너무 많다.
 *    그래서 각 배열에 저장된 리스트의 사이즈를 보고, 비어 있지 않은 것들을 시작 숫자로 저장해둔다.
 *    위의 list와 함께 for문을 돌리면 좋겠지만 선후의 문제가 있음.
 ...
 * 8-4. exploringN에 현재 탐색 중인 값인 next를, exploringI에 그 indext값인 i를 저장한다.
 ...
 *        따라서 뒤로 다시 돌아가야 한다.
 * 8-6-3-1. 현재 진행 중인 숫자와 indext을 버리고, next와 j에는 이전의 값을 입력한다.
 *          뒤로 돌아가므로 movecount를 줄인다.
 *         그 상태로 continue를 하면 이전에 진행 중이던 시점으로 다시 돌아간다. continue로 돌아가면서 j++이 되니까 딱 이전의 시점의 직후부터 이어서 하게 된다.
 ...
 * 8-6-4. 만약 현재 진행 중인 값 arr[next].get(j)가 exploringN에 포함된 값이라면 다음으로 넘어간다.
 * 8-6-5. 이 모든 조건을 무사히 통과했다면 이동할 수 있는 새로운 번호를 발견한 것이다.
 * 8-6-6. next에 현재 진행 중인 값 arr[next].get(j)을 저장해서 다음 회차에서는 숫자와 연관된 숫자를 찾도록 한다.
			...
			for(int j=0; j<arr[next].size()+1; j++) { // 일부러 size+1로 해서 최대값을 size로 하고
				...
				if(j == arr[next].size()) { // j값이 size에 도달하면 어차피 더이상 인덱스가 존재하지도 않고 끝이니까 여기까지 오면 이전 값으로 돌아가도록.
					try {
						exploringN.pop();
						exploringI.pop();
						next = exploringN.pop(); // 그래서 현재 번호를 exploringN에 넣고 이전 숫자로 돌아갈 필요가 있음					
						j = exploringI.pop();
						// 그리고 이전까지 했던 것들은 이미 exploringN에 저장되어 있어서 알아서 건너 뛰고 새로운 걸 찾게 됨.
						// 마지막에 도달했다는 것은 중간에 받아들이는 거 하나 없이 그냥 쭉 갔다는 것
						// 그럼 처음 여기 들어왔을 때 사용한 숫자를 그대로 가지고 있다는 의미임.
						// 그냥 인덱스까지 저장해서 다음 시작 때는 j를 값도 바꿔버려야지 그럼 이렇게 억지로 이전 값을 저장할 필요가 없음
						// j = pop하면 이전 진행하던 것의 다음 순서부터 진행하게 됨
						movecount--;
